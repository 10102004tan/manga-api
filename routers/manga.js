...
router.get("/detail/:slug", async (req, res) => {
  ...
  try {
    ...
    /* Get Chapter List */
    const temp = $("#chapterlist > ul")
    .find("li").toArray().reverse();
    ...
    $(temp)
      ...
      if (index < 20) {
        ...
        var chapter_endpoint = $(el).find("a").attr("href");
        chapter_endpoint = chapter_endpoint.replace("-bahasa-indonesia", "");
        chapter_endpoint = chapter_endpoint.replace("-hq", "");
        let chapter_image = banner
...
// async function getImageLinkFirstForChapter(endpoint){
//   const url = "https://komikindo.pro/";
//   try {
//     const response = await AxiosService(`${url}${endpoint}/`);
//     const $ = cheerio.load(response.data);
//     const getPages = $("#chimg-auh img");
//     return getPages[0].attr("src").replace("i0.wp.com/", "")
//   } catch (error) {
//    console.log(error)
//   }
// }
